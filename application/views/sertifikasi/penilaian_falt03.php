			...
			<div class="card card-custom gutter-b">
				...
				<div class="card-header">
					...
					<div class="card-toolbar">
						...
						<a href="<?= base_url('sertifikasi/falt03/'.$nib_dec.'/'.$tgl_dec.'/'.$user_dec) ;?>"
							target="_blank" style="margin:5px;" class="btn btn-warning font-weight-bolder">
							<i class="flaticon2-cup"></i>FTP03</a>
...
<div class="tab-pane fade" id="peralatan" role="tabpanel"
							...
							<?php echo form_open_multipart(base_url('sertifikasi/insert_penilaian_falt03'), 'method="POST"');?>
								...
								<table style="width:100%; border-collapse:collapse; font-family:Arial, sans-serif; font-size:13px;" border="1">
									...
									<tr style="text-align:center; font-weight:bold;">
										<td colspan="5">HASIL PENILAIAN KELAYAKAN BADAN USAHA</td>
									</tr>
									<!-- Header 1 -->
									<tr style="text-align:center; font-weight:bold; background:#fce9c8;">
										<td rowspan="2">No.</td>
										<td rowspan="2">Item</td>
										<td colspan="2">Hasil Penilaian Kemampuan Penyediaan Peralatan</td>
										<td rowspan="2">REKOMENDASI</td>
									</tr>
									<!-- Header 2 -->
									<tr style="text-align:center; font-weight:bold; background:#fce9c8;">
									...
									<?php
										...
										if(!empty($penilaian_falt03)){
											foreach($penilaian_falt03 as $row_ceklis){
											$count+=1;
											for ($i=0; $i < count($penilaian_falt03); $i++) {
												if($row_ceklis['id']==$penilaian_falt03[$i]['id']){
												${"data_hasil".$row_ceklis['id']}=$row_ceklis['hasil'];
												${"data_persyaratan".$row_ceklis['id']}=$row_ceklis['persyaratan'];
									...
									<tr>
										...
										<td>Jumlah Peralatan</td>
										  <td style="text-align:center;">
											<div class="input-group file-caption-main">
											<span class="file-caption-icon"></span>
											<input type="text" value="<?=$data_hasil1;?>" name="hasil_penilaian" class="form-control">
											</div>
										</td>
										 <td style="text-align:center;">
											<div class="input-group file-caption-main">
											<span class="file-caption-icon"></span>
											<input type="text" value="<?=$data_persyaratan1;?>" name="hasil_persyaratan" class="form-control">
											</div>
										</td>
										<td style="text-align:center;">

													<div class="center">
														<select class="form-control" value="<?=$data_rekomendasi1;?>" name="rekomendasi"  required="required">
															<option <?php if($data_rekomendasi1=='1') :?> checklist="checklist" <?php endif ;?> value="1">SESUAI</option>
															<option<?php if($data_rekomendasi1=='0') :?> checklist="checklist" <?php endif ;?> value="0">TIDAK SESUAI</option>
									...
									<tr>
										...
										<td>Peralatan – <?=$count?>, Jenis, Milik/Sewa, Noreg SIMPK</td>
										<td style="text-align:center;">
											<div class="input-group file-caption-main">
											<span class="file-caption-icon"></span>
											<input type="text" 
											<?php if (empty($penilaian_falt03)) :?>value="<?=$row_peralatan['subvarian'];?>,<?=$row_peralatan['varian'];?>, <?=$row_peralatan['status_peralatan'];?>, <?=$row_peralatan['nomor_registrasi_peralatan'];?>" <?php else :?>
											value="<?=${'data_hasil'.$id_peralatan} ;?>"<?php endif?> name="<?=$id_peralatan;?>hasil_penilaian" class="form-control">
										</div></td>
										 <td style="text-align:center;">
											<div class="input-group file-caption-main">
											<span class="file-caption-icon"></span>
											<input type="text" 
											<?php if(empty($penilaian_falt03)) :?> value="(masukan semua persyaratan peralatan sesuai subklasifikasi baik untuk K, M, B berdasar Permen PUPR No 6/2021, Permen PUPR No 8/2021, Surat Dirjen Binkon dan Surat Dir Usaha dan Kelembagaan)"
											<?php else :?>value="<?=${'data_persyaratan'.$id_peralatan} ;?>" <?php endif ;?> name="<?=$id_peralatan;?>hasil_persyaratan" class="form-control">
										</div></td>
										<td style="text-align:center;">

													<div class="center">
														<select class="form-control" name="<?=$id_peralatan;?>rekomendasi"  required="required">
															<option <?php if(${'data_persyaratan'.$id_peralatan}=='1') :?>selected <?php endif ;?> value="1">SESUAI</option>
															<option <?php if(${'data_persyaratan'.$id_peralatan}=='0') :?>selected <?php endif ;?> value="0">TIDAK SESUAI</option>
														</select>
									...
									<div class="row">
										...
										<div class="input-group file-caption-main">
											<span class="file-caption-icon"></span>
											<textarea id="comment_res" name="catatan_falt03"
												class="form-control form-control-solid" rows="5"
												placeholder="Catatan resuma...">
    <?php if(!empty($catatan_falt03)) :?>
												<?=$catatan_falt03[0]['catatan'];?>	
												<?php else :?>
Catatan :

Berdasarkan ketentuan SK dirjen No. 37 Tahun 2025, pemenuhan penyediaan peralatan Badan Usaha

<?= $biodata[0]['nama'] ;?> pada: Subklasifikasi <?= $klasifikasi[0]['deskripsi_subklasifikasi'] ;?> Kode Subklasifikasi

<?= $klasifikasi[0]['id_sub_klasifikasi'] ;?> Kualifikasi <?= $klasifikasi[0]['kualifikasi'] ;?>, dinyatakan SESUAI / TIDAK SESUAI (hapus yang tidak
...
diperlukan)
								...
								<br>
								<br>
								<button type="submit" name="submit" style="float: right" class="open-delete btn btn-primary btn-labeled btn-rounded"><b><i class="icon-paperplane"></i></b> Submit Penilaian FALT03</button>
