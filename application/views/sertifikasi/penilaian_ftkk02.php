			...
			<div class="card card-custom gutter-b">
				...
				<div class="card-header">
					...
					<div class="card-toolbar">
						...
						<a href="<?= base_url('sertifikasi/ftkk02/'.$nib_dec.'/'.$tgl_dec.'/'.$user_dec) ;?>"
							target="_blank" style="margin:5px;" class="btn btn-success font-weight-bolder">
							<i class="flaticon-customer"></i>FTKK02</a>
...
<div class="col-md-2">
	...
	<div class="tab-pane fade" id="pjbu" role="tabpanel" aria-labelledby="home-tab-1">
			...
			<?php echo form_open_multipart(base_url('sertifikasi/insert_penilaian_ftkk02'), 'method="POST"');?>

											<section class="items">
    <table style="width:100%; border-collapse:collapse; font-family:Arial, sans-serif; font-size:13px;" border="1">
        <!-- 1. PJBU -->
        <tr style="font-weight:bold;">
            <td colspan="6">1. Penanggung Jawab Badan Usaha (PJBU)</td>
        </tr>
   ...
   <div class="center">
	       <select class="form-control" name="rekomendasi_pjbu" required="required">
               <option <?php if($penilaian_ftkk02[0]['pjbu']=='1') :?>selected <?php endif ;?> value="1">SESUAI</option>
			<option <?php if($penilaian_ftkk02[0]['pjbu']=='0') :?>selected <?php endif ;?> value="0">TIDAK SESUAI</option>
                </select>
        ...
        <!-- 2. PJTBU -->
        <tr style="font-weight:bold;">
            <td colspan="10">2. Penanggung Jawab Teknik Badan Usaha (PJTBU)</td>
        </tr>
        <tr style="text-align:center; font-weight:bold; background:#fce9c8;">
            <td style="width:30px;">No</td>
            <td>NAMA</td>
            <td>ALAMAT/KOTA</td>
            <td>KLASIFIKASI BADAN USAHA</td>
            <td>KUALIFIKASI (JENJANG)</td>
            <td>KLASIFIKASI</td>
        ...
        <tr>
            <td style="text-align:center;">1.</td>
            <td><?=$row_pjtbu['nama'];?></td>
            <td><?=$row_pjtbu['alamat'];?></td>
            <td><?=$row_pjtbu['klasifikasi'];?></td>
            <td><?=$row_pjtbu['kualifikasi_skk'];?></td>
            <td><?=$row_pjtbu['klasifikasi_skk'];?></td>
            <td><?=$row_pjtbu['sub_klasifikasi'];?></td>
            <td><?=$row_pjtbu['noreg_skk'];?></td>
            <td><?=$row_pjtbu['noreg_skk'];?></td>
            <td colspan="2" style="text-align:center; vertical-align:middle;">
   <div class="center">
	       <select class="form-control" name="rekomendasi_pjtbu" required="required">
              <option <?php if($penilaian_ftkk02[0]['pjtbu']=='1') :?>selected <?php endif ;?> value="1">SESUAI</option>
			<option <?php if($penilaian_ftkk02[0]['pjtbu']=='0') :?>selected <?php endif ;?> value="0">TIDAK SESUAI</option>
                </select>
        ...
        <!-- 3. PJSKBU -->
        <tr style="font-weight:bold;">
            <td colspan="10">3. Penanggung Jawab Sub Klasifikasi Badan Usaha (PJSKBU)</td>
        </tr>
        <tr style="text-align:center; font-weight:bold; background:#fce9c8;">
            <td colspan="3"></td>
            <td>SUB</td>
            <td colspan="6">SERTIFIKAT KOMPETENSI KERJA</td>
        </tr>
		<tr style="text-align:center; font-weight:bold; background:#fce9c8;">
            <td style="width:30px;">No</td>
            <td>NAMA</td>
        ...
        <tr>
            <td style="text-align:center;">1.</td>
          <td><?=$row_pskbu['nama'];?></td>
            <td><?=$row_pskbu['alamat'];?></td>
            <td><?=$row_pskbu['klasifikasi'];?></td>
            <td><?=$row_pskbu['kualifikasi_skk'];?></td>
            <td><?=$row_pskbu['klasifikasi_skk'];?></td>
            <td><?=$row_pskbu['sub_klasifikasi'];?></td>
            <td><?=$row_pskbu['noreg_skk'];?></td>
            <td><?=$row_pskbu['noreg_skk'];?></td>
           <td colspan="2" style="text-align:center; vertical-align:middle;">
   <div class="center">
	       <select class="form-control"name="rekomendasi_pjskbu" required="required">
            <option <?php if($penilaian_ftkk02[0]['pjskbu']=='1') :?>selected <?php endif ;?> value="1">SESUAI</option>
			<option <?php if($penilaian_ftkk02[0]['pjskbu']=='0') :?>selected <?php endif ;?> value="0">TIDAK SESUAI</option>
    </select>
      ...
      <?php endforeach ;?>
						...
						<input type="hidden" name="id_izin" value="<?=$klasifikasi[0]['id_izin'];?>">
									...
									<div class="row">
										...
										<div class="input-group file-caption-main">
											<span class="file-caption-icon"></span>
											<textarea id="comment_res" name="catatan_ftkk02"
												class="form-control form-control-solid" rows="5"
												placeholder="Catatan resuma...">
													<?php if(!empty($catatan_ftkk02)) :?>
		<?=$catatan_ftkk02[0]['catatan'];?>	
		<?php else :?>
Catatan :

1. Berdasarkan ketentuan Keputusan Dirjen Bina Konstruksi Nomor 37 Tahun 2025 bahwa Persyaratan pemenuhan
Ketersediaan Tenaga Kerja Konstruksi untuk pekerjaan konstruksi bersifat UMUM kualifikasi <?=$klasifikasi[0]['kualifikasi'] ?>
yaitu:

- 1 orang PJBU sebagai pimpinan tertinggi
- 1 orang PJTBU memiliki SKK serendah-rendahnya kualifikasi Jenjang ....... dan subklasifikasi SKK ......... (syaratkualifikasi SKK)
- 1 orang PJSKBU setiap subklasifikasi memiliki serendah-rendahnya kualifikasi Jenjang ....... dan subklasifikasi
SKK ......... (syarat kualifikasi SKK)

2. Badan Usaha <?= $biodata[0]['nama'] ?> memiliki:
- 1 orang PJBU sebagai pimpinan tertinggi dinyatakan SESUAI / TIDAK SESUAI (hapus yang tidak diperlukan)
- 1 orang PJTBU memiliki SKK serendah-rendahnya kualifikasi Jenjang ............ dan subklasifikasi SKK .........dinyatakan SESUAI / TIDAK SESUAI (hapus yang tidak diperlukan)
- 1 orang TK dapat merangkap 5 PJSKBU dalam klasifikasi yang sama, memiliki serendah-rendahnya kualifikasi
Jenjang .......dan subklasifikasi SKK ......... dinyatakan SESUAI / TIDAK SESUAI (hapus yang tidak diperlukan)
<?php endif ;?>
											</textarea>
													...
													<br>
								<br>
								<button type="submit" name="submit" style="float: right" class="open-delete btn btn-primary btn-labeled btn-rounded"><b><i class="icon-paperplane"></i></b> Submit Penilaian FTKK02</button>
