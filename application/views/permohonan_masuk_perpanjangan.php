    ...
    <div class="container">
      ...
      <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
          ...
          <div class="card-toolbar">
            <a  id="proses_data" onclick="javascript:get_permohonan_detail_banyak(this)" class="btn btn-light-dark font-weight-bolder mr-2">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  ...
                  <div class="d-flex align-items-center mr-3" data-inbox="actions">
                    <label class="checkbox checkbox-outline checkbox-outline-2x checkbox-danger mr-3">
                      <input type="checkbox" name="<?= $row['id_izin'] ;?>" value="<?= $row['NIB'] ;?>" <?php if($row['status']=='20') :?>disabled<?php endif ;?> class="call-checkbox"/>
                      <span></span>
                    </label>
                ...
                <td>
                  ...
                  <?php endif ;?>
                  <?php else :?>
                    <a id="<?=$row['NIB'];?>" name="<?=$row['id_izin'];?>" disabled  class="btn btn-icon btn-light-success pulse pulse-primary mr-5">
...
function refresh() {
    ...
    function get_permohonan_detail(sel) {
      ...
      $.ajax({
          url : "<?php echo base_url('sertifikasi/cek_pilihan'); ?>",
          type : "POST",
            ...
            if(response.status=='1' || response.status=='2'){
              Swal.fire({
                    ...
                    if (result.value) {
                      ...
                      $.ajax({
                          url : "<?php echo base_url('sertifikasi/get_permohonan_detail2'); ?>",
                          type : "POST",
                          data : {id_izin : sel.name,
                                  pilihan : response.status},
                          success : function(data) {
                            response = jQuery.parseJSON(data);
              ...
              Swal.fire({
                    ...
                    if (result.value) {
                      ...
                      $.ajax({
                          url : "<?php echo base_url('sertifikasi/get_permohonan_detail_perpanjangan'); ?>",
                          type : "POST",
                          data : {id_izin : sel.name,
    ...
    function get_permohonan_detail_banyak(sel) {
      ...
      rowcollection.each(function(index,elem){
        ...
        Swal.fire({
              title: "Anda ingin get detail permohonan id izin "+coba2.toString()+" ?",
              text: "Proses akan mengambil data detail permohonan badan usaha!",
              icon: "warning",
              input: 'select',
              inputOptions: {
              ...
              if (result.value) {
                ...
                $.ajax({
                    url : "<?php echo base_url('sertifikasi/get_permohonan_coba_perpanjangan'); ?>",
                    type : "POST",
                    data : {id_izin : coba,
                            pilihan : result.value},
                    success : function(data) {
