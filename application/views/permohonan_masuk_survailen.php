          ...
          <table class="table table-bordered table-hover table-checkable mt-10" id="kt_datatable2">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <?php if($row['status']=='103') :?>

                  <a id="<?=$row['NIB'];?>" name="<?=$row['id_izin'];?>"
                    onclick="javascript:get_permohonan_detail(this)"
                    class="btn btn-icon btn-light-success pulse pulse-primary mr-5">
                  ...
                  <a id="<?=$row['NIB'];?>" name="<?=$row['id_izin'];?>"
                    onclick="javascript:get_permohonan_detail(this)"
                    class="btn btn-icon btn-light-danger pulse pulse-primary mr-5">
                ...
                <td>
                  ...
                  <a onclick="javascript:tolak(this)" name="<?= $row['NIB'] ;?>" id="<?= $row['tgl_permohonan'] ;?>"
                    data-toggle="modal" data-target="#modal_tolak" class="btn btn-sm btn-clean btn-icon"
  ...
  function get_permohonan_detail(sel) {
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            $.ajax({
              url: "<?php echo base_url('survailen/get_permohonan_detail_perubahan'); ?>",
              type: "POST",
              data: {
                id_izin: sel.name
              },
