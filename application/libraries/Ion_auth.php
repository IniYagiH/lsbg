...
class Ion_auth
{
	...
	public function ceklogin()
	{
		...
		$session = $CI->User_model->getSession($username);
		$coba = "$session dan $id_login";
		if($status==TRUE AND $session==$id_login){
	...
	public function ceklogin2_survailen()
	{
		...
		$id_login= $this->session->userdata('id_login');
		$status = $this->session->userdata('login');
		$session = $CI->User_model->getSession_survailen($username);
		$coba = "$session dan $id_login";
		if($status==TRUE AND $session==$id_login){
	...
	public function admin_pusat(){
		$id_propinsi = $this->session->userdata('id_propinsi');
		$level = $this->session->userdata('level');
		if($id_propinsi=='00' AND $level=='1'){
			return true;
		}
	}
	public function survailen(){
...
public function admin_rekomendasi(){
		$id_propinsi = $this->session->userdata('id_propinsi');
		$level = $this->session->userdata('level');
		if($level=='10'){
			return true;
	...
	public function pelaksana(){
		$id_propinsi = $this->session->userdata('id_propinsi');
		$level = $this->session->userdata('level');
		if($id_propinsi!='00' AND $level=='2'){
			return true;
		}
	}
	public function koordinator_keuangan(){
		$id_propinsi = $this->session->userdata('id_propinsi');
		$level = $this->session->userdata('level');
		if($id_propinsi=='00' AND $level=='4'){
			return true;
		}
	}
	public function koordinator_sertifikasi(){
		$id_propinsi = $this->session->userdata('id_propinsi');
		$level = $this->session->userdata('level');
		if($id_propinsi=='00' AND $level=='5'){
			return true;
		}
	}
	...
	public function datalogin_survailen()
	{
		$data = $this->User_model->selectwhere_survailen($this->session->userdata('id_user'));
		return $data;
	}
